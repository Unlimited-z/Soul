...
// MARK: - API Models
struct DoubaoAPIRequest: Codable {
    let model: String
...
struct DoubaoResponseMessage: Codable {
    let role: String
    let content: String
}
...
struct DoubaoAPIError: Codable {
    let error: DoubaoErrorDetail
}

struct DoubaoErrorDetail: Codable {
    let message: String
    let type: String?
    let code: String?
}
...
class DoubaoAIService {
    static let shared = DoubaoAIService()
    ...
    // 新增方法：AI 发起对话
    func initiateConversation(systemMessage: String? = nil,
                             completion: @escaping (Result<String, DoubaoAIServiceError>) -> Void) {
...
enum DoubaoAIServiceError: Error, LocalizedError {
    case invalidURL
